    ...
    <div class="bg-white rounded-lg shadow-md p-6 space-y-6 max-w-sm">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-2 p-3 rounded-lg" :class="{
                'bg-green-50 text-green-800': gameStore.isGameActive,
                'bg-gray-50 text-gray-800': !gameStore.isGameActive,
                'bg-red-50 text-red-800': gameStore.isInCheck
            }">
                <div class="w-2 h-2 rounded-full" :class="{
                    'bg-green-500': gameStore.isGameActive && !gameStore.isInCheck,
                    'bg-gray-500': !gameStore.isGameActive,
                    'bg-red-500 animate-pulse': gameStore.isInCheck
                }"></div>
        ...
        <!-- Board Controls -->
        <div class="border-b border-gray-100 pb-4">
            <h4 class="text-md font-medium text-gray-700 mb-3">Brett</h4>

            <div class="flex gap-2">
                <button
                    @click="handleFlipBoard"
                    class="flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg font-medium transition-all duration-200 cursor-pointer bg-gray-200 text-gray-800 hover:bg-gray-300 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    title="Brett drehen"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Export/Import -->
        <div class="border-b border-gray-100 pb-4">
            <h4 class="text-md font-medium text-gray-700 mb-3">Export</h4>

            <div class="flex gap-2">
                <button
                    @click="copyFEN"
                    class="flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg font-medium transition-all duration-200 cursor-pointer border-2 border-gray-300 text-gray-700 hover:border-gray-400 hover:bg-gray-50"
                    title="FEN-String kopieren"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Game Info -->
        <div>
            <h4 class="text-md font-medium text-gray-700 mb-3">Information</h4>

            <div class="space-y-2">
                <div class="flex justify-between items-center py-1">
                    <span class="text-sm text-gray-600">Züge:</span>
                    <span class="text-sm font-medium text-gray-900">{{ gameStore.moveHistory.length }}</span>
                </div>

                <div class="flex justify-between items-center py-1">
                    <span class="text-sm text-gray-600">Status:</span>
                    <span class="text-sm font-medium text-gray-900">{{ gameStore.isGameActive ? 'Aktiv' : 'Beendet' }}</span>
                </div>

                <div v-if="gameStore.gameResult" class="flex justify-between items-center py-1">
                    <span class="text-sm text-gray-600">Ergebnis:</span>
                    <span class="text-sm font-medium text-gray-900">{{ gameStore.gameResult.winner || 'Remis' }}</span>
                </div>
            </div>
        </div>
    </div>
